...
export function ConditionDetailPanel({
  ...
  return (
    <Card
      tone="raised"
      className="mb-6"
      data-testid={`atlas-detail-${condition.id}`}
    >
      <CardHeader>
        <div className="flex items-start justify-between gap-4">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              {condition.name}
              <Badge tone="neutral">{condition.icd10}</Badge>
            </CardTitle>
            <CardDescription>
              {`${condition.patients} patients · Avg improvement `}
              <strong className="text-emerald-600">
                {`+${condition.avgImprovementPct}%`}
              </strong>
            </CardDescription>
          </div>
          <Button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div>
            ...
            <div className="space-y-3">
              {condition.distribution.map((pct, i) => (
                <div key={i}>
          ...
          <div>
            <h4 className="text-[11px] font-medium uppercase tracking-[0.14em] text-text-subtle mb-4">
              {`Top products for ${condition.name}`}
            </h4>
            <div className="space-y-3">
                    ...
                    <div className="flex-1 h-1.5 bg-surface-muted rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(
                            100,
                            (p.usage / condition.patients) * 100 * 1.5
                          )}%`,
                        }}
                      />
                    </div>
...
export function AtlasView({ conditions }: { conditions: Condition[] }) {
  ...
  return (
    <div data-testid="atlas-grid" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-8">
      {conditions.map((c) => {
        const isExpanded = active?.id === c.id;
        return (
          <React.Fragment key={c.id}>
            ...
            >
              <Card
                tone="raised"
                className={cn(
                  "transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 hover:border-accent/40 h-full w-full",
                  isExpanded && "border-accent ring-2 ring-accent/30"
                )}
              >
                <CardHeader>
