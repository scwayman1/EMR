# Modern Folio Storybook Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Transform the "My Story" fairytale block into a premium, interactive split-screen "Modern Folio" layout with sleek animations.

**Architecture:** We will install `framer-motion` for fluid page transitions. We will rewrite `storybook-view.tsx` into a stateful client component that tracks the `activeChapter`. The layout will feature a two-pane grid (`FolioSpread`): the left pane acts as a sticky art/title canvas with dynamic gradients, and the right pane displays the text with a massive drop-cap.
...
git commit -m "chore: install framer-motion for storybook folio animations"
```

---

### Task 2: Refactor `page.tsx` container width

To support a two-pane split screen, the page container needs to be wider than `700px`.

**Files:**
- Modify: `src/app/(patient)/portal/storybook/page.tsx`

- [ ] **Step 1: Expand the PageShell width**

In `src/app/(patient)/portal/storybook/page.tsx`, change the `PageShell` `maxWidth` prop from `max-w-[700px]` to `max-w-5xl`.
...
- [ ] **Step 2: Update the `prose-clinical` boundaries**

Since the `PageShell` is now wider, we should constrain the text on the data-heavy chapters so they don't stretch too wide. Find all instances of `<div className="prose-clinical">` or `<div className="prose-clinical space-y-4">` and add `max-w-[700px]`.

```tsx
// Example modification:
// <div className="prose-clinical space-y-4 max-w-[700px]">
```

- [ ] **Step 3: Commit**

```bash
git add src/app/\(patient\)/portal/storybook/page.tsx
git commit -m "style: widen storybook page shell for folio layout"
```

---

### Task 3: Implement the Folio Storybook View

**Files:**
- Modify: `src/app/(patient)/portal/storybook/storybook-view.tsx`

- [ ] **Step 1: Completely replace `storybook-view.tsx`**

Replace the contents of `src/app/(patient)/portal/storybook/storybook-view.tsx` with the new Modern Folio architecture.
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="w-full max-w-5xl mx-auto bg-white rounded-3xl shadow-xl overflow-hidden border border-border/40 relative min-h-[600px] flex flex-col md:flex-row print:shadow-none print:border-none print:block">
        ...
        {/* Top Progress & Tools */}
        <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-center z-10 print:hidden">
          <div className="bg-white/80 backdrop-blur-md px-3 py-1.5 rounded-full shadow-sm border border-border/50">
        ...
        <div className="flex-[1.2] p-12 md:p-20 flex flex-col justify-center relative bg-white overflow-hidden print:p-0">
          ...
          <div className="absolute bottom-8 right-8 md:bottom-12 md:right-12 flex gap-3 print:hidden">
            <Button
              variant="secondary"
              size="icon"
              className="rounded-full h-12 w-12 shadow-sm border-border-strong/30"
              onClick={gotoPrev}
              disabled={isFirst}
            >
...
```bash
git add src/app/\(patient\)/portal/storybook/storybook-view.tsx
git commit -m "feat: implement modern folio layout for storybook"
```
